import { journeyRates } from './pipeline-metrics.mjs?v=2026-09-08-customer-endpoint';
...
export function calculateAntonyPlan({actual={},goal={}}={}) {
 ...
 const potentialCloserAppointments=multiply(nonNegative(actual.appointments),effectiveRates.appointmentToCloser);
 ...
 const gap=(required,actual)=>required===null?null:Math.max(0,required-nonNegative(actual));
 return {currentRates,effectiveRates,requiredCustomers,customersForRevenue,requiredSales,requiredDecidedCloserCalls,requiredCloserCalls,requiredCloserAppointments,requiredAppointments,
 ...
 achievedRevenueCents:customerValueCents>0?nonNegative(actual.newCustomers)*customerValueCents:null,
 gaps:{appointments:gap(requiredAppointments,actual.appointments),closerAppointments:gap(requiredCloserAppointments,actual.closerAppointments),decidedCloserCalls:gap(requiredDecidedCloserCalls,actual.decidedCloserCalls),customers:gap(requiredCustomers,actual.newCustomers)}};
}
