...
import {METRICS,DEFAULT_METRICS,metricFacts,metricEntries,monthlyBounds,personLabel} from './verified-journey.mjs?v=2026-09-10-separate-groups';
...
export function renderJourneyChart(report,enabled=DEFAULT_METRICS) {
 ...
 const shortLabel=k=>({setting:'Setting · Verlauf',closing:'CC1 · Verlauf'})[k]||METRICS[k].label;
 ...
 return '<div class="journey-chart">'+legend+'<p class="journey-chart-help">'+(trend?'Leads je Monat · jeder Monat separat':'Leads im Zeitraum · Punkt = geänderter KPI-Stand · gestrichelte Linie = Monatsaktivität')+'</p><p class="journey-scroll-hint">Graph nach rechts wischen →</p><div class="lead-history-scroll"><svg class="lead-history-svg journey-history-svg" viewBox="0 0 1000 360" role="group" aria-label="Gemeinsamer KPI-Verlauf"><text x="48" y="22">Anzahl Leads</text>'+grid+shapes+hits+endLabels+(!keys.length?'<text x="500" y="170" text-anchor="middle">Kennzahl oben auswählen</text>':'')+'</svg></div><p class="selection-basis">'+(trend?'Jeder Monat separat; laufender Monat unvollständig.':'Verlauf innerhalb des Zeitraums bis zum Datenstand.')+' Belegte Gespräche der rückblickenden Gruppe · Monatsaktivität und Neukunden separat · Klick auf Verlauf oder Endwert zeigt Zeitraum und Berechnungsbasis.</p></div>';
}
