...
import {METRICS,DEFAULT_METRICS,metricFacts,metricEntries,monthlyBounds,personLabel} from './verified-journey.mjs?v=2026-09-10-verified-journey';
const fmt=new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',dateStyle:'short',timeStyle:'short'});
const date=v=>Number.isFinite(Date.parse(v))?fmt.format(new Date(v))+' Uhr':'—';
...
export function renderJourneyReport(report,key='setting',dimension='lead_source',series=DEFAULT_METRICS) {
 ...
 '<section class="selection-outcomes"><div class="selection-heading"><h3>Konversion derselben Leads</h3></div><div class="selection-statuses">'+transitions.map(r=>rateButton(report,r)).join('')+'</div><p class="selection-basis">Zähler und Nenner verfolgen dieselben rückblickend ausgewerteten Leads bis zum Datenstand. Neue Buchungen („Entered“) sind keine abgeschlossenen Setter. Offene Follow-ups bleiben offen.</p><details class="selection-method"><summary>Show-, No-Show- und Verlustquoten</summary><div class="selection-statuses">'+otherRates.map(r=>rateButton(report,r)).join('')+'</div><p>Gesprächs- und No-Show-Nachweise stammen aus veröffentlichten Aktivitäten derselben Leads. „Ohne No Show“ allein ist kein Gesprächsnachweis.</p></details></section>'+
