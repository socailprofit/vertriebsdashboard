...
import {METRICS,DEFAULT_METRICS,metricFacts,metricEntries,monthlyBounds,personLabel} from './verified-journey.mjs?v=2026-09-10-shared-cohort';
...
export function renderJourneyChart(report,enabled=DEFAULT_METRICS) {
 ...
 const hits=points.map((p,i)=>{
  const point={title:trend?p.label:'KPI-Verlauf',time:date(p.start)+' – '+date(p.end),rows:keys.map(k=>({label:METRICS[k].label,value:p.values[k]+' '+METRICS[k].unit})),note:'Pipeline: eine gemeinsame rückblickende Leadgruppe aus Setting-/Closing-Statuswechseln, belegtem CC2 oder erstem Neukundenereignis im Zeitraum. Gesprächsverlauf seit dem zugehörigen Setting-Eintritt, einschließlich Vormonaten. Monatsaktivität: Gespräche innerhalb des angezeigten Monats. Setting-Auswahl UTC, Closing Berlin; Neukunden beim ersten passenden Statuswechsel, unabhängig vom Monat früherer Phasen. Berlin. '+(trend?'Jeder Monat separat.':'Kumuliert innerhalb des ausgewählten Zeitraums.')};
 ...
 return '<div class="lead-history-legend">'+Object.entries(METRICS).map(([k,m])=>'<label style="--series-color:'+m.color+'"><input type="checkbox" data-lead-chart-series="'+k+'" '+(enabled.includes(k)?'checked':'')+'>'+esc(m.label)+'</label>').join('')+'</div><div class="lead-history-scroll"><svg class="lead-history-svg" viewBox="0 0 1000 300" role="group" aria-label="Gemeinsamer KPI-Verlauf"><text x="48" y="15">Anzahl Leads</text>'+grid+shapes+hits+'</svg></div><p class="selection-basis">'+(trend?'Jeder Monat separat; laufender Monat unvollständig.':'Verlauf innerhalb des Zeitraums bis zum Datenstand.')+' Belegte Gespräche der rückblickenden Gruppe · Monatsaktivität und Neukunden separat · Klick zeigt Zeitraum und Berechnungsbasis.</p>';
}
export function renderJourneyReport(report,key='setting',dimension='lead_source',series=DEFAULT_METRICS) {
 ...
 '<section class="selection-outcomes"><div class="selection-heading"><h3>Konversion derselben Leads</h3></div><div class="selection-statuses journey-transitions">'+transitions.map(r=>rateButton(report,r)).join('')+'</div><p class="selection-basis">Alle Stufen stammen aus derselben rückblickenden Leadgruppe. Übergangsquoten verwenden die belegten Gespräche der Ausgangsstufe als Nenner. Zugehörige Setting- und Closing-Gespräche aus Vormonaten bleiben enthalten. Neue Buchungen („Entered“) sind keine abgeschlossenen Setter. Offene Follow-ups bleiben offen.</p><details class="selection-method"><summary>Show-, No-Show- und Verlustquoten</summary><div class="selection-statuses">'+otherRates.map(r=>rateButton(report,r)).join('')+'</div><p>Gesprächsnachweise stammen aus veröffentlichten Aktivitäten. No Shows sind durch Aktivitäten oder ausdrückliche Statuswechsel derselben Leads belegt. „Ohne No Show“ allein ist kein Gesprächsnachweis.</p></details></section>'+
 ...
 '<div class="selection-heading"><p>'+esc(group.label)+' · '+leads.length+' Leads · belegter Verlauf · heutige CRM-Zuordnung</p><button type="button" class="selection-detail" data-lead-evidence="all">Alle '+leads.length+' Leads ↗</button>'+(['setting','closing'].includes(group.key)?'<button type="button" class="selection-detail" data-lead-evidence="cohort">Gemeinsame Ausgangsgruppe: '+group.source_total+' inkl. No Shows ↗</button>':'')+'</div>'+
 ...
 '<p>Close-Referenzen: '+(reference?.leads.length||0)+' eindeutige Leads mit Old Status „Setting“, '+(closingReference?.leads.length||0)+' mit Old Status „Closing“. Diese Auswahlen werden mit belegten CC2- und Neukundenereignissen zu einer gemeinsamen Leadgruppe vereinigt. Das sind keine neuen Buchungen. Setting-Referenz: UTC; Closing-Referenz: Europe/Berlin; jeweils Date created. Gesprächsaktivitäten und Neukunden werden nach Europe/Berlin ausgewertet. Die Close-Spalte Ended zeigt den Statusbestand und belegt keine durchgeführten Gespräche.</p>'+
