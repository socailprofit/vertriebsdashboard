...
export const journeyDimensions={overview:'Gesamtübersicht',companies:'Unternehmen · gewählte Phase',lead_source:'Herkunft / Leadquelle',industry_wz:'Branche / Unterbranche (WZ)',opener:'Opener',working_capital:'Working Capital',liquidity_factor:'Liquiditätsfaktor',liquidity_statement:'Liquiditäts-Aussage',backoffice:'Kanal / Backoffice'};
function dimensionName(lead,key) {
 const d=lead.dimensions||{};
 if(key==='liquidity_factor'){const f=liquidity(lead);return f.label+' · '+f.note;}
...
export function renderJourneyReport(report,key='setting',dimension='overview',series=DEFAULT_METRICS) {
 ...
 return '<div class="selection-sources journey-pipeline" aria-label="Pipeline">'+report.groups.map(g=>{
  const conversation=['setting','closing'].includes(g.key);
  const noShows=conversation?metricEntries(report,g.key==='setting'?'setter_no_show':'closer_no_show',report.activity_start,report.activity_end).length:0;
  return '<button type="button" class="selection-source '+(g.key===group.key?'is-active':'')+'" data-lead-source="'+g.key+'" aria-pressed="'+(g.key===group.key)+'"><span>'+(g.key==='setting'?'Setter-Gespräche':g.label)+(g.key==='cc2'?' · optional':'')+'</span><strong>'+(conversation?g.monthly_entries.length:g.total)+'</strong><small>'+(conversation?'Gespräche im Zeitraum · belegte Leads':esc(sub[g.key]))+'</small>'+(conversation?'<div class="journey-source-context"><span><b>'+noShows+'</b> No Shows im Zeitraum</span><span><b>'+g.source_total+'</b> Leads in der CRM-Auswahl · inkl. No Shows</span></div>':'')+'</button>';
 }).join('')+'</div>'+
 '<p class="selection-basis">Setting und Closing: tatsächlich belegte Gespräche im gewählten Zeitraum. No Shows zählen separat. Die CRM-Auswahl zählt Statuswechsel aus der Phase, keine Gespräche.</p>'+
 (prefix?'<div class="selection-attendance journey-period-measures"><button type="button" data-lead-evidence="monthly"><span>Gesprächsnachweise im Zeitraum ansehen</span><strong>'+group.monthly_entries.length+' '+(group.monthly_entries.length===1?'Lead':'Leads')+' ↗</strong></button><button type="button" data-lead-evidence="unknown"><span>CRM-Auswahl · Teilnahme ungeklärt</span><strong>'+group.audit.unknown.length+' Leads ↗</strong></button></div>':'')+
