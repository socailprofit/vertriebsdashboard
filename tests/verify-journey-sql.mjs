...
await db.exec(`alter table close_funnel_events add column event_type text;
create table public.sales_people(close_user_id text,slug text);
create function public.get_reporting_calendar_internal(text,date) returns jsonb language sql as $$select '[]'::jsonb$$;
drop function public.reconcile_close_funnel_snapshot(timestamptz);
create function public.reconcile_close_funnel_snapshot(p_snapshot_started_at timestamptz,p_status_created_since timestamptz) returns jsonb language plpgsql as $$begin
perform 1 from public.close_funnel_events e where (e.source_kind<>'lead_status_change' or (e.payload->>'date_created')::timestamptz>=p_status_created_since);
return '{}'::jsonb;end;$$;`);
await db.exec(fs.readFileSync(new URL('../supabase/migrations/20260910074228_verified_journey_cohorts.sql',import.meta.url),'utf8'));
const current=await report('month','2026-09-10');
assert.equal(current.version,'2026-09-10.verified-journey');
assert.equal(current.leads.length,4);
assert(current.activity_history.some(e=>e.lead_id==='lead_D'&&e.status_id===customer));
assert(!current.activity_history.some(e=>e.source_event_id==='future'));
assert.equal(new Date(current.activity_start).toISOString(),'2026-08-31T22:00:00.000Z');
const patched=(await db.query("select pg_get_functiondef('public.reconcile_close_funnel_snapshot(timestamptz,timestamptz)'::regprocedure) d")).rows[0].d;
assert.match(patched,/status_history_complete_at/);
await db.query("update close_funnel_leads set lead_source='LinkedIn',opener_close_user_id='former' where lead_id='lead_A'");
assert.equal((await db.query("select get_close_opener_internal('lead_A') x")).rows[0].x,'outside_current_team');
await db.query("update close_funnel_leads set report_dimensions=report_dimensions||'{\"backoffice_id\":\"linkedin\",\"backoffice_basis\":\"verified CRM field\"}'::jsonb where lead_id='lead_A'");
assert.equal((await db.query("select get_close_opener_internal('lead_A') x")).rows[0].x,'linkedin');
await db.exec("set test.allowed='false'");await assert.rejects(report('month','2026-09-10'),/Nicht berechtigt/);
assert.equal((await db.query("select has_function_privilege('anon','get_antony_lead_selection_report(text,date)','execute') x")).rows[0].x,false);
assert.equal((await db.query("select has_function_privilege('authenticated','get_antony_lead_selection_sources_internal(text,date)','execute') x")).rows[0].x,false);
console.log('Verified journey SQL: full history, exact source bounds, independent customers, safe reconciliation scope, no source-based attribution, authorization.');
await db.close();
