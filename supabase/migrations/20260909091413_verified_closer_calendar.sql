...
with bounds as (
 ...
 when s.meeting_id is not null then coalesce(o.outcome,'unknown') when proof.stage is not null then 'attended' else 'unknown' end outcome,
 ...
 left join lateral (
 select min(e.process_id) process_id,case when bool_or(e.event_type='cc2_sold' or exists(select 1 from public.close_process_events prior where prior.process_id=e.process_id and prior.removed_at is null and prior.event_type='cc2_agreed' and prior.occurred_at<m.starts_at)) then 'cc2' else 'closer' end stage
 ...
 where e.removed_at is null and e.event_type in ('closer_completed','closer_sold','cc2_sold','cc2_agreed','closer_follow_up','closer_lost')
