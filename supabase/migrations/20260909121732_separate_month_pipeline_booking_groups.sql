...
with source as materialized (
 ...
 d->>'opened_at' booking_at,case when d->>'opened_at' is null then 'unknown' when date_trunc('month',(d->>'opened_at')::timestamptz at time zone 'Europe/Berlin')=date_trunc('month',p_reference_date::timestamp) then 'new' when (d->>'opened_at')::timestamptz < date_trunc('month',p_reference_date::timestamp) at time zone 'Europe/Berlin' then 'carryover' else 'unknown' end booking_scope,
