...
-- Minimal calendar archive, independent of the rolling activity cleanup.
-- No attendee names, emails, descriptions, transcripts or meeting links.
create table public.close_meetings (
 meeting_id text primary key, lead_id text, contact_id text, owner_id text,
 starts_at timestamptz not null, ends_at timestamptz not null,
 date_created timestamptz not null, date_updated timestamptz not null,
 status text, participant_ids jsonb not null default '[]',
 calendar_event_uids text[] not null default '{}', excluded_purpose boolean not null default false,
 booking_activity_id text unique, booking_owner_id text,
 last_seen_at timestamptz not null, removed_at timestamptz,
 check(ends_at>=starts_at), check(jsonb_typeof(participant_ids)='array'),
 check((booking_activity_id is null)=(booking_owner_id is null))
);
create index close_meetings_start on public.close_meetings(starts_at) where removed_at is null;
create index close_meetings_lead_start on public.close_meetings(lead_id,starts_at) where removed_at is null;
alter table public.close_meetings enable row level security;
revoke all on public.close_meetings from public,anon,authenticated;
grant all on public.close_meetings to service_role;
...
begin
 ...
 update public.close_meetings set booking_activity_id=null,booking_owner_id=null;
