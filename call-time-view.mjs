...
import { analyzeCallTimeWindows, calculateCallTimeQuality, callTimeMetric,
  callTimeHours, CALL_TIME_MIN_CALLS, CALL_TIME_MIN_CONTACTS } from './call-time-score.mjs?v=2026-09-18-hour-evidence';
...
function pointPayload(person, entry, mode, period) {
  ...
  return {
    ...
    rows: [
      ...
      { label: 'Dokumentierte Opening-/Follow-up-Aktivitäten', value: number(q.opening_activities) },
      ...
      { label: 'Entscheider (erreicht / dokumentierte Aktivitäten)', value: ratio(q.rates.decision, q.decision_maker_contacts, q.opening_activities) },
    ...
    note: `${q.inconsistent.length ? 'Widersprüchliche Zähler und Bezugsgrößen: Dieses Fenster wird nicht empfohlen. ' : ''}${!entry.enoughData ? 'Für eine Empfehlung fehlen ausreichend dokumentierte Fälle. ' : ''}Anrufe: tatsächliche Close-Anrufzeit. Gesprächsergebnisse: Ereigniszeit der Opening-/Follow-up-Dokumentation; ohne Verknüpfung kein Nachweis der Anrufstunde. Termine zählen zum Zeitpunkt ihrer Dokumentation, nicht zum späteren Termin. Alle Zeiten Europe/Berlin. Fehlende Stufen ergeben keinen Gesamtscore.`,
  };
}
...
export function renderCallTimeProfile(rows = [], people = [], mode = 'quality', fallbackPeriod = '') {
  ...
  const hours = callTimeHours(rows.filter(row=>people.some(p=>p.slug===row.slug)));
  ...
  const recommendations = `<div class="hour-recommendations" aria-label="Zwei beste Zeitfenster je Person">
    ${analyses.map((analysis) => `<section class="hour-person-picks" style="--person-color:${safeColor(analysis.person.color)}">
      ...
      ${analysis.ranked.length < 2 ? `<p class="hour-pick-empty">${analysis.ranked.length ? 'Noch kein zweites' : 'Noch kein'} Zeitfenster mit ausreichenden Nachweisen für eine Empfehlung.</p>` : ''}
    </section>`).join('')}
  </div>`;
  ...
  const matrix = [...hours].sort((a, b) => a - b).map((hour) => `<div class="hour-row"><span class="hour-label">${hourLabel(hour)}</span><span class="hour-bars">${analyses.map((analysis) => {
    ...
    return `<button type="button" class="hour-bar ${missing ? 'is-missing' : ''} ${!missing && !entry.enoughData ? 'is-thin' : ''} ${rank ? 'is-best' : ''}"
      ...
      <small class="hour-meta">${number(entry.quality.calls_gross)} Anrufe · ${number(entry.quality.productive_calls)} produktiv · ${number(entry.quality.decision_maker_contacts)} Entscheider · ${number(entry.quality.appointments)} Termine</small>
    </button>`;
  }).join('')}</span></div>`).join('');
  const rule = mode === 'quality'
    ? `Mindestens ${CALL_TIME_MIN_CALLS} Anrufe und ${CALL_TIME_MIN_CONTACTS} dokumentierte Fälle in jeder der drei Gesprächsstufen je Zeitfenster.`
  ...
  return `<p class="hour-pick-empty">Uhrzeiten in Deutschland · Anrufe nach Anrufbeginn, Gesprächsergebnisse nach dokumentierter Ereigniszeit. Sichtbar ist der Zeitraum mit erfasster Aktivität.</p>` + recommendations + head + matrix + `<details class="hour-rules"><summary>Berechnungsregel · ${escapeHtml(metricName)}</summary>
    ...
    <p>Gesamtqualität: 35 % produktive Erreichbarkeit, 25 % Durchstellung, je 20 % Entscheider- und Terminquote. Nur bei vier berechenbaren Stufen; fehlende Stufen werden nicht durch höhere Gewichte ersetzt. Punkte beschreiben den beobachteten Zeitraum und sind keine Vorhersage.</p>
    <p>Entscheiderquote: erreichte Entscheider geteilt durch dokumentierte Opening-/Follow-up-Aktivitäten derselben Stunde. Durchstellung und Terminquote nutzen ebenfalls ausschließlich diese Dokumentationen; sie werden nicht durch die separat erfassten Telefonanrufe geteilt.</p>
