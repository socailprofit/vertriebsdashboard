import { escapeHtml as esc } from './render-security.mjs?v=2026-09-10-lead-selections';
...
export function renderSelectionReport(report,key='setting',dimension='lead_source') {
 ...
 <p class="selection-basis">Statusanteile je Zeile beziehen sich auf die Leads dieser Zeile.</p>
...
export function renderLeadEvidence(report,key,dimension,type,value) {
 ...
 const all=uniqueLeads(group);let leads=all,label='Alle Leads';
 ...
 return `<div class="selection-heading"><div><p class="eyebrow">${esc(group.label)}</p><h3 id="lead-evidence-title">${esc(label)}</h3></div><button type="button" data-close-lead-dialog aria-label="Details schließen">Schließen ×</button></div><p>${leads.length} von ${all.length} Leads · ${pct(leads.length,all.length)} der ${esc(group.label)}-Auswahl</p>
