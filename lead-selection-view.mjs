...
export function renderLeadEvidence(report,key,dimension,type,value) {
 ...
 const fields=['lead_source','industry_wz','opener','working_capital','liquidity_statement'];
 ...
 <div class="selection-table-wrap"><table class="selection-table"><thead><tr><th>Lead / aktueller Status</th>${fields.map(k=>`<th>${esc(dimensions[k])}</th>`).join('')}<th>Statuswechsel erfasst</th></tr></thead><tbody>${leads.map(l=>`<tr><th>${/^lead_[A-Za-z0-9]+$/.test(l.lead_id)?`<a href="https://app.close.com/lead/${esc(l.lead_id)}/" target="_blank" rel="noopener noreferrer">${esc(l.lead_name)} ↗</a>`:esc(l.lead_name)}<small>${esc(l.status_label)}</small></th>${fields.map(k=>`<td>${esc(name(l,k))}${k==='working_capital'&&l.dimensions?.financials_date?`<small>Stand: ${esc(l.dimensions.financials_date)}</small>`:''}</td>`).join('')}<td>${esc(date(l.first_recorded_at))}${Number(l.matching_events)>1?`<small>${Number(l.matching_events)} passende Wechsel · zuletzt ${esc(date(l.last_recorded_at))}</small>`:''}</td></tr>`).join('')||'<tr><td colspan="7">Keine Leads.</td></tr>'}</tbody></table></div>`;
}
