...
const ms=Date.parse;
// All retrospective paths share the same upper bound, including defensive client checks.
export function reportCutoff(report,end=report.activity_end) {
 return Math.min(...[end,report.activity_end,report.data_as_of].map(ms).filter(Number.isFinite),Date.now());
}
const SETTER_RESULTS=new Set(['✅ Closer terminiert','❌ Disqualifiziert','🔎 Setter Follow Up']);
...
export function journeyFacts(report) {
 const end=reportCutoff(report), all=report.activity_history||[], facts=[];
 ...
 for(const e of events) {
  ...
  if(e.event_type==='setter_activity') {
   if(!SETTER_RESULTS.has(e.setter_result)){push(e,'setter_unconfirmed');continue;}
   push(e,'setter_show');
 ...
 for(const m of report.calendar||[]) {
  if(!Number.isFinite(ms(m.starts_at))||ms(m.starts_at)>=end)continue;
...
export const METRICS=Object.freeze({
 setting_source:{label:'Setting · CRM-Auswahl',color:'#4c7beb',unit:'Leads'},
 closing_source:{label:'CC1 · CRM-Auswahl',color:'#b77839',unit:'Leads'},
 setting:{label:'Setting · davon Gespräch belegt',color:'#79a2ff',unit:'Leads'},
 closing:{label:'CC1 · davon Gespräch belegt',color:'#e7a14c',unit:'Leads'},
 setter_show:{label:'Setter-Gespräche im Zeitraum',color:'#8dc5ff',unit:'Leads'},
 cc1_show:{label:'CC1-Gespräche im Zeitraum',color:'#f3bf69',unit:'Leads'},
...
export const DEFAULT_METRICS=['setting_source','setting','setter_show','cc1_show','cc2_show','customer'];
...
export function selectedCohort(report,key,start,end) {
 const status=STATUS[key],cutoff=Math.min(ms(end),ms(report.data_as_of),Date.now());
 const events=(report.activity_history||[]).filter(e=>e.source_kind==='lead_status_change'&&e.previous_status===status&&ms(e.recorded_at)>=ms(start)&&ms(e.recorded_at)<cutoff)
...
export function metricEntries(report,key,start,end) {
 end=new Date(reportCutoff(report,end)).toISOString();
 const sourceOnly=key.endsWith('_source');if(sourceOnly)key=key.replace('_source','');
...
export function buildJourneyReport(raw,filters={}) {
 ...
 const report={...raw,activity_end:new Date(reportCutoff(raw)).toISOString(),leads,activity_history:raw.activity_history.filter(e=>ids.has(e.lead_id)),calendar:(raw.calendar||[]).filter(m=>ids.has(m.lead_id)&&Number.isFinite(ms(m.starts_at))&&ms(m.starts_at)<reportCutoff(raw))};
 ...
 report.groups=[['setting','Setting','setting'],['closing','Closer Call 1','closing'],['cc2','Closer Call 2','cc2_show'],['customer','Neukunden','customer']].map(([key,label,metric])=>{
  ...
  const cohort=reference&&['setting','closing'].includes(key)?withPhaseEntries(report,key,selectedCohort(report,key,reference.selection_start,reference.selection_end)):entries;
  const monthly_entries=metricEntries(report,key==='setting'?'setter_show':key==='closing'?'cc1_show':metric,report.activity_start,report.activity_end);
  const showIds=new Set(entries.map(e=>e.lead_id));
  const noShows=cohort.filter(c=>report.facts.some(f=>f.lead_id===c.lead_id&&f.key===(key==='setting'?'setter_no_show':'closer_no_show')&&ms(f.at)>=ms(c.at)&&ms(f.at)<reportCutoff(report)));
  const noShowIds=new Set(noShows.map(c=>c.lead_id));
  const audit={noShows,overlap:cohort.filter(c=>showIds.has(c.lead_id)&&noShowIds.has(c.lead_id)),unknown:cohort.filter(c=>!showIds.has(c.lead_id)&&!noShowIds.has(c.lead_id))};
  ...
  return {key,label,metric,audit,monthly_entries,total:entries.length,leads:groupLeads,cohort,source_total:cohort.length,
